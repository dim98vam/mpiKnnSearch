...
//function to add two vectors, first one of size 1xM and the second one of 1xn creating an
...
knnresult kNNSequential(double* X,double* Y,int* indexes,int n,int m,int d,int k){
        ...
        int i;
        for(i=0;i<m;++i)
            memcpy(&index[n*i],indexes,n*sizeof(int));
        ...
        for (gap = nextGap(gap); gap > 0; gap = nextGap(gap))
        {
            ...
            for (i = 0; i + gap < n; i++)
                if (arr1[i] > arr1[i + gap]) {
                    SWAPDouble(arr1[i],arr1[i + gap]);
                    SWAPint(kIndexes[i],kIndexes[i+gap]);
                }

            // comparing elements in both arrays.
            for (j = gap > n ? gap - n : 0 ;i < n && j < m; i++, j++)
                if (arr1[i] > arr2[j]) {
                    SWAPDouble(arr1[i],arr2[j]);
                    SWAPint(kIndexes[i],indexes[j]);
                }
            ...
            if (j < m)
            {
                ...
                for (j = 0; j + gap < m; j++)
                    if (arr2[j] > arr2[j + gap]){
                        SWAPDouble(arr2[j],arr2[j + gap]);
                        SWAPint(indexes[j],indexes[j+gap]);
                    }
            }
        }
    }
...
void kNNMpi(double* X,double* Y,double* kNeighborsDist,int* indx ,int n,int m,int d,int k,knnresult* holder){
        ...
        for(i=0;i<m;++i){
            ...
            merge((holder->ndist)+i*k,distanceMatrix+i*n,(holder->nidx)+i*k,index+i*n,k,k);
        }
}
